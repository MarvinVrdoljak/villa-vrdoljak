...
root.find(".thb-field-blockslist").each(function(){$(this).thbBlocksListField();});if(root.is(".thb-field"))
root.thbSlide();else
root.find(".thb-field").each(function(){$(this).thbSlide();});if(root.is(".thb-field-section"))
root.thbSection();else
root.find(".thb-field-section").each(function(){$(this).thbSection();});if(root.is(".thb-field-background")){var bg=new THB_FieldBackground($(this));}
else{root.find(".thb-field-background").each(function(){var bg=new THB_FieldBackground($(this));});}
$(window).trigger("thb_boot_fields",[root]);};})(jQuery);(function($){$.fn.thbBlocksListField=function(){if(!$(this).parents(".thb-modal").length){return;}
var modal=$(this).parents(".thb-modal").data("modal"),type=$(this).find("tr"),input=$(this).find("input");type.on("click",function(){input.val($(this).data("type"));modal.close();return false;});};})(jQuery);(function($){window.THB_FieldBackground=function(field){var overlay_display=field.find("[name*='[overlay_display]']"),overlay_color=field.find(".thb-overlay-color"),overlay_opacity=field.find(".thb-overlay-opacity"),overlay=field.find(".thb-overlay");overlay_color.minicolors({change:function(hex){overlay.css("background-color",hex);}});overlay_opacity.on("change",function(){overlay.css("opacity",overlay_opacity.val());});overlay_display.on("change",function(){var checked=$(this).attr("value");if(checked=='1'){overlay.show();}
else{overlay.hide();}});overlay.css("background-color",overlay_color.val());overlay.css("opacity",overlay_opacity.val());overlay.css("display",overlay_display.val()=='1'?'block':'none');};})(jQuery);(function($){$.fn.thbTabField=function(){var placeholder=$(this).find(".tab-item-placeholder"),item=$(this).find(".tab-item");placeholder.on("click",function(){item.toggle();$(this).parent().toggleClass('open');return false;});};})(jQuery);(function($){$.fn.thbPricingTableField=function(){var placeholder=$(this).find(".pricingtable-item-placeholder"),item=$(this).find(".pricingtable-item");placeholder.on("click",function(){item.toggle();$(this).parent().toggleClass('open');return false;});};})(jQuery);(function($){window.thb_builder_add_section=function(container,tpl,control){if(control.hasClass("thb-section-add-section")){var field=container.addField(tpl,control.parents(".thb-field-section"));}
...
break;}
return placeholder;};var THB_Section=function(field){var self=this,columnTemplate=THB_Template($('script[data-tpl="column_create"]').html()),rowTemplate=THB_Template($('script[data-tpl="row_create"]').html()),blockTemplate=THB_Template($('script[data-tpl="block_create"]').html()),rowsContainer=field.find(".thb-rows-container");var sizes=['one-fifth','one-fourth','one-third','two-fifths','one-half','three-fifths','two-thirds','three-fourths','four-fifths','full'];var n_sizes=['1/5','1/4','1/3','2/5','1/2','3/5','2/3','3/4','4/5',thb.strings.full];var p_sizes=[0.2,0.25,0.333,0.4,0.5,0.6,0.666,0.75,0.8,1];this.addRow=function(animate){var row=$(rowTemplate());rowsContainer.append(row);self.bindRow(field,row);thb_boot_fields(row);if(animate){row_position=row.offset().top-$('body').offset().top-20;$.scrollTo(row_position,500);}
...
el.trigger("change");return false;});});};})(jQuery);(function($){$.fn.thbSelect=function(){return this.each(function(){$(this).customSelect();var select=$(this),custom=select.next(".customSelect");if(select.data("callback")!=""){select.on("change.thb_select",function(){var fn=window[select.data("callback")];if(typeof fn==='function'){fn(select.val());}});}
$(window).on("resize.thb_select",function(){select.css("width",custom.outerWidth());select.css("height",custom.outerHeight());});setTimeout(function(){$(window).trigger("resize");},10);});};})(jQuery);(function($){$.fn.thb_serialize=function(){var elements=$(this).find(':input').get(),done=[],ret=[];$.each(elements,function(){if(this.name&&!this.disabled&&(this.checked||/select|textarea/i.test(this.nodeName)||/text|number|hidden|password/i.test(this.type))){var val=$(this).val();if($.inArray(this.name,done)<0){done.push(this.name);ret.push({"name":this.name,"value":val});}}});return ret;};})(jQuery);function thb_remove_all_duplicable_items(container,tpl){container.removeAllFields();return false;}
